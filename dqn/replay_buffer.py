...
class ExpReplay():
    ...
    def add(self, state, action, reward, next_state, done):
        ...
        Args:
            ...
            done (bool): Wether next_state doesn't represents an ending state
        """
        ...
        # save in the memory wether a state is NOT final to ease computation in the agent
        self.done_mem[idx] = 1 - int(done)

        self.pointer += 1

    def sample(self, batch_size=64):
        """Get a batch of samples using prioritized experience replay
        ...
        # Calculate the sampling probability through proportional prioritization and normalize it
        sampling_probability = (self.losses[:max_mem]+self.epsilon)**self.alpha
        sampling_probability /= sampling_probability.sum()
        
        # Get sample's indexes from memory
