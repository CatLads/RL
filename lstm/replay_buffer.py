...
class ExpReplay():
    ...
    def sample(self, batch_size=64):
        ...
        states = self.state_mem[chosen_element:(chosen_element+batch_size)]
        actions = self.action_mem[chosen_element:(chosen_element+batch_size)]
        rewards = self.reward_mem[chosen_element:(chosen_element+batch_size)]
        next_states = self.next_state_mem[chosen_element:(chosen_element+batch_size)]
        dones = self.done_mem[chosen_element:(chosen_element+batch_size)]

        return states, actions, rewards, next_states, dones, [i for i in range(chosen_element, chosen_element+batch_size)]
